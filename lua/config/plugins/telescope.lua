return {
    ...
    config = function()
        ...
        vim.keymap.set('n', '<leader>lf', [["zy<cmd>exec 'Telescope file_browser path=%:p:h select_buffer=true no_ignore=true'<CR>]],
            { desc = "File browser" })
        vim.keymap.set('x', '<leader>sh', [["zy<cmd>exec 'Telescope grep_string default_text=' . escape(@z, ' ')<cr>]],
